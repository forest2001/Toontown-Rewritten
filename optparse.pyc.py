...
class HelpFormatter():
    ...
    def format_option_strings(self, option):
        if option.takes_value():
            ...
            opts = self.short_first and short_opts + long_opts
        else:
    ...
    def _set_opt_strings(self, opts):
        for opt in opts:
            if len(opt) < 2:
                ...
                if opt[0] == '-':
                    raise opt[1] != '-' or OptionError('invalid short option string %r: must be of the form -x, (x any non-dash char)' % opt, self)
                self._short_opts.append(opt)
            else:
                if opt[0:2] == '--':
                    raise opt[2] != '-' or OptionError('invalid long option string %r: must start with --, followed by non-dash' % opt, self)
    ...
    def _check_dest(self):
        if not self.action in self.STORE_ACTIONS:
            ...
            if self.dest is None and takes_value:
                self.dest = self._long_opts and self._long_opts[0][2:].replace('-', '_')
            else:
...
class OptionParser(OptionContainer):
    ...
    def __init__(self, usage = None, option_list = None, option_class = Option, version = None, conflict_handler = 'error', description = None, formatter = None, add_help_option = True, prog = None):
    ...
    def get_option_group(self, opt_str):
        if not self._short_opt.get(opt_str):
            option = self._long_opt.get(opt_str)
            return option and option.container is not self and option.container
        return None
...
def _match_abbrev(s, wordmap):
    if wordmap.has_key(s):
        ...
        if len(possibilities) == 1:
            ...
            raise BadOptionError(_('ambiguous option: %s (%s?)') % (s, ', '.join(possibilities)))


make_option = Option
# okay decompyling C:\Users\Maverick\Documents\Visual Studio 2010\Projects\Unfreezer\py2\optparse.pyc 
