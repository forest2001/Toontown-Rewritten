...
# Embedded file name: direct.distributed.DistributedCamera
...
class Fixture(NodePath, FSM):
    ...
    def __str__(self):
        return "Fixture(%d, '%s', %s, %s, %s)" % (self.id,
         self.state,
         self.getPos(),
         self.getHpr(),
         self.getFov())
    ...
    def setRecordingInProgress(self, inProgress):
        ...
        if self.recordingInProgress and base.config.GetInt('camera-id', -1) >= 0:
            self.hide()
        else:
            self.show()

    def show(self):
        if base.config.GetBool('aware-of-cameras', 0) and not self.recordingInProgress:
            NodePath.show(self)
...
class DistributedCamera(DistributedObject):
    ...
    def stopTesting(self, index):
        ...
        if fixture:
            ...
            localAvatar.b_setGameState('LandRoam')
# okay decompyling C:\Users\Maverick\Documents\Visual Studio 2010\Projects\Unfreezer\py2\direct\distributed\DistributedCamera.pyc 
