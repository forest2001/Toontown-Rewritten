...
# Embedded file name: direct.showbase.BpDb
import inspect
import sys
...
class BpDb():
    ...
    def bp(self, id = None, grp = None, cfg = None, iff = True, enabled = True, test = None, frameCount = 1):
        if self.enabled:
            if not self.verifyEnabled():
                return
            return enabled and (iff or None)
    ...
    def bpCall(self, id = None, grp = None, cfg = None, iff = True, enabled = True, test = None, frameCount = 1, onEnter = 1, onExit = 0):
        ...
        if self.enabled:
            if not self.verifyEnabled():
                return decorator
            if enabled:
                return iff or decorator
            bpi = bp(id=id, grp=grp, cfg=cfg, frameCount=frameCount + 1)
            return bpi.disabled and decorator
    ...
    def __init__(self, id = None, grp = None, cfg = None, frameCount = 1):
        ...
        if not grp:
            if not className:
                ...
                if not id:
                    self.id = lineNumber
                    cfg = cfg is None and moduleName and moduleName.lower()
                    cfg = cfg.find('distributed') == 0 and cfg[len('distributed'):]
            self.cfg = cfg
            self.disabled = bpdb.verifyConfig(self.cfg) or True
            return
        return
...
# okay decompyling C:\Users\Maverick\Documents\Visual Studio 2010\Projects\Unfreezer\py2\direct\showbase\BpDb.pyc 
