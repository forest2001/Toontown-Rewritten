...
class DistributedPlayer(DistributedAvatar.DistributedAvatar, PlayerBase.PlayerBase, TelemetryLimited):
    ...
    def setLocation(self, parentId, zoneId):
        ...
        if parentId in (0, None):
            if not zoneId in (0, None):
                self.cr._isValidPlayerLocation(parentId, zoneId) or self.cr.disableDoId(self.doId)
    ...
    def getAutoRun(self):
        return self.autoRun
# okay decompyling C:\Users\Maverick\Documents\Visual Studio 2010\Projects\Unfreezer\py2\otp\avatar\DistributedPlayer.pyc 
