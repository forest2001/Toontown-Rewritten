...
QuestDialogDict = {160: {GREETING: '',
 ...
 1081: {QUEST: '_toNpcName_ appears to be in a sticky situation...\x07If you help him out, he says can give you a big boost._where_'},
...
def getLastSeenString(timestamp):
    ts = datetime.datetime.fromtimestamp(timestamp).strftime(DATEFORMAT)
    date = datetime.datetime.strptime(ts, DATEFORMAT)

    current_datetime = datetime.datetime.now()
    delta = str(current_datetime - date)
    ...
    if days >= 7:
        xdays = days
        datelets.append('%d week%s' % (xdays, plural(xdays)))
    ...
    # This is where it turns to shit
    if (minutes == 0 and xdays == 0 and months == 0 and years == 0) and float(seconds) == 0:
        return "Just now"
    elif (minutes == 0 and xdays == 0 and months == 0 and years == 0) and float(seconds) < 60:
        return "Less than a minute ago"

    return ', '.join(datelets) + ' ago.' # Would return something like "1 hour and 15 minutes ago"
